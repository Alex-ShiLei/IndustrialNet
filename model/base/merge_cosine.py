...
class merge(nn.Module):
    ...
    def forward(self, qur_sups, sims,diffs,gt=None):
        ...
        for s in range(self.shot):
            simS = sims[s]
            simList = []
            for l in range(len(simS)):  # L4,L3,L2
        ...
        x4 = self.conv4(self.simShotConv4(corr_lySim[0]/self.shot))

        x3 = self.conv3(self.simShotConv3(corr_lySim[1]/self.shot))

        x2 = self.conv2(self.simShotConv2(corr_lySim[2]/self.shot))
