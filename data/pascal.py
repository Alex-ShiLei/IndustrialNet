...
class DatasetPASCAL(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize):
        ...
        self.img_metadata,self.img_metadata_val = self.build_img_metadata()
    ...
    def build_img_metadata(self):
        ...
        if self.split == 'trn':  # For training, read image-metadata of "the other" folds
            ...
            val_metadata+=read_metadata('trn', self.fold)
        ...
        return img_metadata,val_metadata

    def build_img_metadata_classwise(self):
        img_metadata_classwise = {}
        img_metadata_classwise_val =set([items[0] for items in self.img_metadata_val])
        meta_data=[]
        for class_id in range(self.nclass):
            img_metadata_classwise[class_id] = []

        for img_name, img_class in self.img_metadata:
            if img_name in img_metadata_classwise_val:
                continue
