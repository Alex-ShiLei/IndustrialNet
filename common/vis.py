...
class Visualizer:
    ...
    @classmethod
    def visualize_prediction_batch(cls, spt_img_b, qry_img_b, qry_mask_b, pred_mask_b, cls_id_b, batch_idx, iou_b=None):
        spt_img_b = utils.to_cpu(spt_img_b)
        sp_size=spt_img_b.size()
        spt_mask_b =torch.zeros([sp_size[0],sp_size[-2],sp_size[-1]],dtype=torch.long) #utils.to_cpu(spt_mask_b)
